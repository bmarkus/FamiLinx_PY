...
void computeidcoefs(const char *outfile)
{
  ...
  //double idcoef[NIDSTATE];
   ...
   for (i=0; i<Npairs; i++) {
     int minped_err = 0;
      minped_err = minimalped(2, Pair[i]);
      //printminped();
      ...
      if (minped_err) {
	bugflag = 1;
	fprintf(outfl,"%d\t%d\n", getfindiv(Pair[i][0]), getfindiv(Pair[i][1]));
      }
   ...
   if (bugflag) {
     fprintf(stderr, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
     fprintf(stderr, "! Your pedigree appears to trip a bug present in Idcoefs versions <= 2.1\n");
     fprintf(stderr, "! Pairs whose identity coefficients may be in error are listed in the \n");
     fprintf(stderr, "! file %s. I recommend rerunning these pairs using Idcoefs\n", outfile);
     fprintf(stderr, "! version 2.1.1 or later.\n");
     fprintf(stderr, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
   }
}
