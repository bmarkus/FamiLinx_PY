...
int minimalped(int nsample, int *samplelist)
/* Find the minimal pedigree connecting the people in the sample list.
   ...
   In this bug check version, check for array overflow by counting fidx
   and returning the value 1 if fidx > npeople and 0 otherwise.
*/
{
   ...
   for (i=0; (fid=Founderlist[i]) != 0; i++) {
     //if ( !Inminped[fid]) continue; // This fixes the bug.
